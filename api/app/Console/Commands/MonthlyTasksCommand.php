...
class MonthlyTasksCommand extends Command {
    protected $signature = "recurring:monthly";
    protected $description = "Starts montly tasks";
    ...
    public function handle(){
        $date = new DateTime();
        $date = $date->format("D");
            $startedTasks = StartedTasks::where('timeframe', '=', 'monthly')->get('task_id')->toArray();
            $startedTaskIds = array();
            foreach($startedTasks as $startedTask){
                $startedTaskIds[] = $startedTask['task_id'];
            }
            $data = RecurringTasksModel::where('recurring', '=', 'monthly')->whereNotIn('id', $startedTaskIds)->get()->toArray();
            foreach($data as $task){
                $taskDesc = "Monthly task: " . $task['task'];
                try {
