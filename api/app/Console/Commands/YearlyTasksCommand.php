...
class YearlyTasksCommand extends Command {
    protected $signature = "recurring:yearly";
    protected $description = "Starts yearly tasks";
    ...
    public function handle(){
        $date = new DateTime();
        $date = $date->format("D");
            $startedTasks = StartedTasks::where('timeframe', '=', 'yearly')->get('task_id')->toArray();
            $startedTaskIds = array();
            foreach($startedTasks as $startedTask){
                $startedTaskIds[] = $startedTask['task_id'];
            }
            $data = RecurringTasksModel::where('recurring', '=', 'yearly')->whereNotIn('id', $startedTaskIds)->get()->toArray();
            foreach($data as $task){
                $taskDesc = "Yearly task: " . $task['task'];
                try {
